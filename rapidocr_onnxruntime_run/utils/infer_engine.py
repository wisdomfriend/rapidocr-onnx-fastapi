...
HIP_INDEX = int(os.getenv("OCR_HIP_INDEX", "0"))
...
class OrtInferSession:
    ...
    def _get_ep_list(self) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        rocm_provider_opts = {
            "device_id": self.hip_index
        }
        self.use_cuda = self._check_cuda()
        if self.use_cuda:
            EP_list.insert(0, (EP.ROCM_EP.value, rocm_provider_opts))
    ...
    def _check_cuda(self) -> bool:
        ...
        if cur_device == "GPU" and EP.CUDA_EP.value in self.had_providers:
            return True
        if cur_device == "GPU-MIGRAPHX" and EP.ROCM_EP.value in self.had_providers:
            return True
